...
def show_venue(venue_id):
	...
	for show in shows:
		if show.venue_id == data['id']:
			new_show = show.__dict__
			del new_show['_sa_instance_state']
			artist = Artist.query.get(show.artist_id)
			new_show['artist_name'] = artist.name
			new_show['artist_image_link'] = artist.image_link
			if dateutil.parser.parse(show.start_time)<datetime.now(pytz.utc):
...
def show_artist(artist_id):
	...
	for show in shows:
		if show.artist_id == data['id']:
			...
			new_show['venue_image_link'] = venue.image_link
			if dateutil.parser.parse(show.start_time)<datetime.now(pytz.utc):
				past_shows.append(new_show)
			else:
				upcoming_shows.append(new_show)
...
def shows():
	...
	for show in shows:
		...
		data.append({
			...
			'venue_name': venue.name if None!=venue else None,
			'artist_id': show.artist_id,
			'artist_name': artist.name if None!=artist else None,
